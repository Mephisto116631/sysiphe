    ...
    if tous_les_exos:
        ...
        if select_exos:
            ...
            if not df_muscu.empty:
                df_muscu["volume"] = df_muscu["performance"]
...
def render_theme_tab():
    ...
    choix = st.radio("Thème", list(THEMES.keys()), index=0)
...
def render_stats_tabs(df_global: pd.DataFrame, tous_les_exos: list, user_id: str) -> None:
    ...
    tab1, tab2, tab3 = st.tabs(["📈 Graphiques", "🎨 Apparence", "⚙️ Paramètres"])
    with tab1: render_graph_tab(df_global, tous_les_exos, st.session_state.include_planche, current_theme)
    with tab2: render_theme_tab()
    with tab3: st.write("Paramètres ici")
