"""
Sysiphe — Onglets de statistiques (Graphiques, Records, Repos, Paramètres).
"""
...
def _plateau_badge(dates, values, label: str) -> None:
    ...
    if result["status"] == "progress":
        ...
        st.warning(f"📉 {label} : en baisse sur les 21 derniers jours — envisage un deload ou du repos.")
    elif result["status"] == "plateau":
        st.info(f"➖ {label} : stable sur les 21 derniers jours (plateau).")
    # 'insufficient_data' -> pas assez d'historique, on n'affiche rien
...
def render_graph_tab(df_global: pd.DataFrame, tous_les_exos: list, include_planche: bool) -> None:
    ...
    min_db_date, max_db_date = df_global["date"].min(), df_global["date"].max()
    ...
    # --- Planche : ffill (pas d'interpolation fictive entre séances espacées) ---
    if include_planche:
        ...
        if not df_planche.empty:
            ...
            if not pivot_p.empty:
                ...
                df_best_par_jour = df_planche.groupby("date")["effort_pondere"].max().reset_index()
                _plateau_badge(df_best_par_jour["date"], df_best_par_jour["effort_pondere"], "Planche")

    # --- Musculation : tonnage (kg) si charge renseignée, sinon reps ---
    if tous_les_exos:
        ...
        if select_exos:
            ...
            if not df_muscu.empty:
                df_muscu = df_muscu.copy()
                df_muscu["volume"] = df_muscu.apply(
                    lambda r: r["performance"] * r["charge"] if r.get("charge", 0) > 0 else r["performance"],
                    axis=1,
                )
    ...
    # --- Volume par catégorie : zero fill (journalier) ---
    ...
    if not pivot_cat.empty:
        ...
        fig_c = px.line(df_melt_c, x="date", y="Séries", color="Catégorie", title="Nombre de séries par catégorie")
...
def render_calendar_tab(df_global: pd.DataFrame) -> None:
    """Calendrier annuel façon GitHub : intensité = nombre de séries loggées par jour."""
    ...
    activity = df_global.groupby("date").size().to_dict()  # nb de séries par jour
    df_cal = compute_daily_activity(activity, annee_sel)

    total_series = int(df_cal["count"].sum())
    nb_jours_actifs = int((df_cal["count"] > 0).sum())
    st.caption(f"**{annee_sel} : {total_series} séries enregistrées sur {nb_jours_actifs} jour(s) d'entraînement**")

    ncols = int(df_cal["week_index"].max()) + 1
    z = np.full((7, ncols), np.nan)
    text = np.full((7, ncols), "", dtype=object)

    for _, row in df_cal.iterrows():
        w, wk = int(row["weekday"]), int(row["week_index"])
        z[w, wk] = row["count"]
        text[w, wk] = f"{row['date'].strftime('%d %b %Y')}<br>{int(row['count'])} série(s)"
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        colorscale=[[0, "#161b22"], [0.01, "#0e4429"], [0.35, "#006d32"], [0.65, "#26a641"], [1, "#39d353"]],
        showscale=False, xgap=3, ygap=3, zmin=0,
    ))
    fig.update_layout(
        yaxis=dict(
        ...
        height=220, margin=dict(l=40, r=20, t=40, b=10),
    ...
    with c1:
        ...
        if not df_p.empty:
            ...
            df_pr = (df_p.drop_duplicates(subset=["variante", "elastique", "tension"])
                    [["date", "variante", "elastique", "tension", "performance", "effort_pondere"]].copy())
            df_pr.columns = ["Date", "Variante", "Élastique", "Tension", "Temps (s)", "Effort"]
            df_pr[["Temps (s)", "Effort"]] = df_pr[["Temps (s)", "Effort"]].astype(int)
            st.dataframe(df_pr.style.background_gradient(subset=["Effort", "Temps (s)"], cmap="YlOrRd"),
    ...
    with c2:
        ...
        if not df_m.empty:
            df_m["volume"] = df_m.apply(
                lambda r: r["performance"] * r["charge"] if r.get("charge", 0) > 0 else r["performance"], axis=1
            )
            ...
            df_pr_muscu = (df_vol.loc[idx_max]
            ...
            st.dataframe(df_pr_muscu.style.background_gradient(subset=[label], cmap="Blues"),
                        use_container_width=True, hide_index=True)
        else:
...
def render_repos_tab(df_global: pd.DataFrame) -> None:
    ...
    with cr1:
        ...
        fig_pie = px.pie(habitudes, values="Nb", names="Jours", title="Répartition de tes formats de repos")
        st.plotly_chart(fig_pie, use_container_width=True)

    with cr2:
        fig_bar = px.bar(df_repos_stats, x="date_dt", y="jours_repos", title="Chronologie des jours de repos")
        fig_bar.update_layout(xaxis_title="", yaxis_title="Jours de repos consécutifs")
        fig_bar.update_traces(marker_color="#1f77b4", hovertemplate="Reprise : %{x}<br>Repos : %{y} jours")
        ...
        st.dataframe(df_log.sort_values("Date de Reprise", ascending=False),
                    use_container_width=True, hide_index=True)
...
def render_param_tab(df_global: pd.DataFrame, tous_les_exos: list, user_id: str) -> None:
    ...
    st.caption("Ces scores pondèrent la difficulté de chaque variante. Sauvegardés sur ton compte, "
    ...
    if st.button("✅ Appliquer et sauvegarder la calibration", use_container_width=True):
        ...
        if ok:
            ...
            st.warning("Scores appliqués pour cette session, mais la sauvegarde a échoué "
        ...
        with col_btn:
            ...
            if st.button("Renommer", use_container_width=True):
                ...
                if not nv:
                    ...
                    st.error(f"⚠️ '{nv}' existe déjà. Supprime ou fusionne d'abord.")
                else:
                    try:
